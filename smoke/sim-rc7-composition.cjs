// rc.7 真实组合冒烟（rc.7 改造 16.5）：不 mock 运行内核——直接装载已安装的 rc.7
...
// ① rc.7 Loader/Runner 是否接受生成的 payload（真实 precheckCode + vm sandbox 求值）；
...
//
// 如实说明的边界：Client 半无法在 Node 内装载（rc.7 client 模块依赖 window.__ModuleLoader__
// 与页面远端连接，npm 发行版不含可编程 Web 组合）——本套件以「产物核对」替代：断言所安装
// rc.7 的 sidebar/layout 包真实声明 sidebar.footer.action / shell.overlay 两个 Slot。
//
// 版本门：要求全局 dsh 与 host-runner 均为 0.1.0-rc.7（rc.6 或混装依赖树 → fail loud）。
const fs = require('fs')
...
;(async () => {
  ...
  // ---- 版本门：rc.7 精确匹配；rc.6/混装 fail loud ----
  ...
  if (dshPkg.version !== EXPECTED_VERSION) {
    console.log('>>> 版本门未通过，终止（本冒烟按 rc.7 契约断言）')
    process.exit(1)
  }

  // ---- 装载真实 rc.7 模块（绝对路径 import；裸依赖在该树内自行解析）----
  ...
  // ---- Slot 产物核对：rc.7 sidebar/layout 包真实声明工具箱使用的两个 Slot ----
  ...
  check('rc.7 存在 sidebar.footer.action Slot', sidebarBundle.indexOf('sidebar.footer.action') >= 0)
  check('rc.7 存在 shell.overlay Slot', layoutBundle.indexOf('shell.overlay') >= 0)
  ...
  // ---- 能力检查（16.4 同构断言）：rc.7 runner 具备工具箱依赖的方法面 ----
  ...
  check('run 返回 awaiting-approval（rc.7 异步状态机）', Boolean(runRes && runRes.ok && runRes.status === 'awaiting-approval'), JSON.stringify(runRes))
